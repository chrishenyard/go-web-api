...
func Test_UserCanAuthenticateWithPKCE(t *testing.T) {
	...
	secrets, err := config.GetVaultSecrets([]string{"KEYCLOAK_USER_PASSWORD", "OIDC_CLIENT_SECRET"})
	if err != nil {
		t.Fatalf("get Vault secrets: %v", err)
	}

	password := secrets["KEYCLOAK_USER_PASSWORD"]
